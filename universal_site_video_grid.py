...
def resolve_max_url_length(max_url_length: int, sample_url: Optional[str]) -> int:
    if sample_url:
        normalized = normalize_url(sample_url)
        if normalized:
            return len(normalized)
        return len(sample_url)
...
async def run_crawl(args: argparse.Namespace) -> int:
    ...
    if not parsed_base.scheme:
        base = "https://" + base
    args.max_url_length = resolve_max_url_length(args.max_url_length, args.sample_url_length_from)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sample-url-length-from",
        default="",
        help="Sample URL to derive max URL length automatically",
    )
...
def build_gui() -> None:
    ...
    sample_url_length_var = tk.StringVar()
    ...
    def run_crawl_from_gui(use_cached: bool = False) -> None:
        ...
        args.sample_url_length_from = sample_url_length_var.get().strip()
